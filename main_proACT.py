...
# from source_analysis import (forward_model,get_epochs_dics,source2atlas)
import changepath
import pickle
import mne
...
def do_preprocessing(subjects_dir='/envau/work/comco/ficarella.s/etudeMEG/forFrioul/', subject='sb1', session='1'):
    '''
...
'''def create_source_model(subjects_dir='/hpc/comco/ficarella.s/db_mne/meg_te/', subject='sb4'):
...
def compute_singletrial_source_power(subjects_dir='/hpc/comco/brovelli.a/db_mne/meg_te/', subject='subject_04',
    ...
    # Plotting image
    pow.average().plot_image(picks=np.arange(0, 41, 1), units='z-score', scalings=1, titles='HGA - lh',
                             cmap='interactive')
    plt.yticks(np.linspace(0, 40, 41), pow.ch_names[0:41], rotation='horizontal', fontsize=8)
    plt.grid()
    pow.average().plot_image(picks=np.arange(41, 82, 1), units='z-score', scalings=1, titles='HGA - lr',
                             cmap='interactive')
